    ...
    def get_img_and_seg(self, gt_seg):
        ...
        if '_20' in patient:
            ...
            if scanname is None:
                ...
                return
        img_nib = nib.load(os.path.join('src/'+Scan_Folder,scanname))
        seg_nib = nib.load(os.path.join('src/'+GT_Folder,'Final_'+gt_seg))
...
#         print(Z.shape)
        ...
        #X -= np.mean(X)
        #X /= np.std(X)
        return img_info, torch.from_numpy(X).type(dtype), torch.from_numpy(Y).type(dtype).long(), torch.from_numpy(Z).type(dtype), scanname
...
writer = SummaryWriter(comment='-no_prob_conv')
...
for fold, (train_ids, test_ids) in enumerate(kfold.split(nph_dataset)):
    ...
    trainloader = torch.utils.data.DataLoader(
                      nph_dataset, num_workers=25, pin_memory=True, batch_size=batch_size,
                      persistent_workers=10, prefetch_factor=2, sampler=train_subsampler)
    testloader = torch.utils.data.DataLoader(
                      nph_dataset, num_workers=8, pin_memory=True, 
                      batch_size=4, sampler=test_subsampler)
        ...
        if l%10==0: 
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(trainloader, 0):
                    ...
                    #dice
                    outputs = get_mask_from_output(outputs, info)
                ...
                loss_track_train.append(avg_loss_train/num_train)
                dice_cerebral_train.append(avg_dice_cerebral_train/num_train)
                dice_vent_train.append(avg_dice_vent_train/num_train)
                dice_sub_train.append(avg_dice_sub_train/num_train)


                print('\nTRAIN LOSS SUMMARY')
                print('-'*50)
                print(
                    f' Train: Average Loss            {avg_loss_train/num_train}\n',
                    f'Train: Cerebral Dice Score     {avg_dice_cerebral_train/num_train}\n',
                    f'Train: Ventricle Dice Score    {avg_dice_vent_train/num_train}\n',
                    f'Train: Subcortical Dice Score  {avg_dice_sub_train/num_train}',
                )
                print('-'*50)


                writer.add_scalar('Train: Average Loss', avg_loss_train/num_train, l)
                writer.add_scalar('Train: Average Cerebral Dice Score', avg_dice_cerebral_train/num_train, l)
                writer.add_scalar('Train: Average Subcortical Dice Score', avg_dice_sub_train/num_train, l)
                writer.add_scalar('Train: Ventricle Dice Score', avg_dice_vent_train/num_train, l)
                ...
                loss_track_test.append(avg_loss_test/num_test)
                dice_vent_test.append(avg_dice_vent_test/num_test)
                dice_cerebral_test.append(avg_dice_cerebral_test/num_test)
                dice_sub_test.append(avg_dice_sub_test/num_test)
                epochs_per_score.append(l)

                writer.add_scalar('Test: Average Loss', avg_loss_test/num_test, l)
                writer.add_scalar('Test: Average Cerebral Dice Score', avg_dice_cerebral_test/num_test, l)
                writer.add_scalar('Test: Average Subcortical Dice Score', avg_dice_sub_test/num_test, l)
                writer.add_scalar('Test: Ventricle Dice Score', avg_dice_vent_test/num_test, l)


                print('\nTEST LOSS SUMMARY')
                print('-'*50)
                print(
                    f' Test: Average Loss            {avg_loss_test/num_test}\n',
                    f'Test: Cerebral Dice Score     {avg_dice_cerebral_test/num_test}\n',
                    f'Test: Ventricle Dice Score    {avg_dice_vent_test/num_test}\n',
                    f'Test: Subcortical Dice Score  {avg_dice_sub_test/num_test}',
                )
                print('-'*50)
